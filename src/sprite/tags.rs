...
#[derive(Clone, Debug)]
pub struct ExistingTags<S: TagSource> {
    sprite_id: &'static str,
    tag_names: HashSet<String>,
    pd: PhantomData<S>,
}

impl<S: TagSource> ExistingTags<S> {
...
impl RepresentibleTags {
    ...
    pub fn with<S: TagSource>(mut self) -> Self {
        ...
        let file_pattern = format!("./assets/{}/{}_*.png", self.sprite_id, source_name);
        let tag_subsets = glob::glob(&file_pattern)
            ...
            .map(|(stem, tx_file)| {
                let repr_tags: HashSet<String> =
                    stem.split("_").map(|t| t.to_owned()).skip(1).collect();
                (repr_tags, tx_file)
            })
            .collect_vec();
